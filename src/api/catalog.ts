...
export async function fetchCatalog(identifier: string): Promise<CatalogResponse> {
  ...
  const { data: store, error: storeError } = await supabase
    .from("stores")
    .select("id, name, slug, logo_url, banner_url, description, whatsapp, phone, address, delivery_info, custom_domain, coupons_enabled, instagram, facebook, schedule, primary_color")
