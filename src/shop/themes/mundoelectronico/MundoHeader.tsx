...
import { Search, ShoppingCart, Instagram, ChevronLeft, ChevronRight } from "lucide-react";
...
const SLIDES = [
    { id: 1, imageUrl: `${CDN}/hero1.webp`,    alt: 'Reparación de celulares' },
    { id: 2, imageUrl: `${CDN}/notebooks.webp`, alt: 'Notebooks & PCs' },
];
...
function HeroSlider() {
    ...
    useEffect(() => {
        const t = setInterval(() => {
            setFading(true);
            setTimeout(() => { setCurrent(c => (c + 1) % SLIDES.length); setFading(false); }, 300);
        }, 5000);
    ...
    return (
        <div style={{ position: 'relative', lineHeight: 0 }}>
            <img
                src={slide.imageUrl}
                alt={slide.alt}
                style={{ width: '100%', display: 'block', opacity: fading ? 0 : 1, transition: 'opacity 0.3s ease' }}
            />

            <button onClick={prev}
                className="absolute left-3 top-1/2 -translate-y-1/2 w-9 h-9 flex items-center justify-center rounded-full hover:opacity-80 transition-opacity"
                style={{ backgroundColor: 'rgba(0,0,0,0.35)', color: WHITE, zIndex: 10 }}>
                <ChevronLeft className="w-5 h-5" />
            </button>
            <button onClick={next}
                className="absolute right-3 top-1/2 -translate-y-1/2 w-9 h-9 flex items-center justify-center rounded-full hover:opacity-80 transition-opacity"
                style={{ backgroundColor: 'rgba(0,0,0,0.35)', color: WHITE, zIndex: 10 }}>
